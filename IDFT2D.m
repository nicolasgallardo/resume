% DSP Project 1
% 3/18/2016
% This function performs a 2-D IDFT. Here the only difference from the DFT
% is the frequency value being used.

function idft = IDFT2D(x)
...
idft = zeros(size(P,1),size(P,2));
for k = 1:N2_rows
    for l = 1:N2_cols
        for b = 1:N2_cols
% Notice we are traversing along each row by holding the row and indexing
% the column in P(k,b)
            idft(k,l) = P(k,b)*exp(1i*2*pi*(l-1)*(b-1)/N2_cols)+idft(k,l);
        end
    end
end
end
